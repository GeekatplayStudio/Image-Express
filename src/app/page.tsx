...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      const localModeEnabled = localStorage.getItem('image-express-local-mode') === '1';
      if (desktopDetected) {
        setUsername(storedUser || 'Local Desktop');
        setDisplayName('Local Desktop');
        setUserRoles(['admin']);
        setShowLoginModal(false);
      } else if (!storedUser && localModeEnabled) {
        // Local-use mode: single-machine usage without an account. Keep the
        // Guest identity so asset/design APIs that treat "Guest" as the
        // no-auth owner keep working; Google is only needed for Drive.
        setUsername('Guest');
  ...
  const handleLocalUse = () => {
    localStorage.setItem('image-express-local-mode', '1');
    setUsername('Guest');
    setDisplayName('Local User');
    setUserRoles(['admin']);
    setShowLoginModal(false);
