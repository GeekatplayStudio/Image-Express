...
export async function POST(req: NextRequest) {
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      if (meshEntry instanceof File && meshEntry.size > 0) {
        meshFile = meshEntry;
      }
      ...
      if (hitemsSupportsPbr(model)) {
        fields.pbr = normalizeHitemsPbr(model, getString(form.get('pbr'))) ? '1' : '0';
      }
      if (hitemsRequiresMeshUrl(requestType) && !meshUrl && !meshFile) {
      ...
      if (multiImagesBit) fields.multi_images_bit = multiImagesBit;
      if (hitemsSupportsPbr(model)) {
        fields.pbr = normalizeHitemsPbr(model, getBodyString(body?.pbr)) ? '1' : '0';
      }
