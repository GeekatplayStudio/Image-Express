            ...
            <div className="flex items-center justify-between gap-2">
                <div>
                    ...
                    <div className="text-[10px] text-muted-foreground">
                        Official templates: {snapshot?.serverTemplates.length || 0} | Workflow files: {snapshot?.customFolderWorkflows.length || 0}
                    </div>
                    <div className="text-[10px] text-muted-foreground">
                        Reuse official ComfyUI templates and scanned workflow JSON files for the current task.
                    </div>
                </div>
                <button
                    type="button"
            ...
            <div className="max-h-48 space-y-2 overflow-y-auto pr-1">
                {visibleEntries.length === 0 ? (
                    <div className="text-[10px] text-muted-foreground rounded border border-dashed border-border/60 px-2 py-3">
                        No runnable official templates or workflow-folder entries were discovered yet. Connect ComfyUI, or add workflow folder(s) in Settings.
                    </div>
                ) : (
                        ...
                        <div key={`${entry.source}:${entry.id}:${entry.location || ''}`} className="rounded border border-border/60 bg-secondary/10 px-2 py-2">
                            <div className="flex items-start justify-between gap-2">
                                <div className="min-w-0">
                                    ...
                                    <div className="text-[10px] text-muted-foreground">
                                        {entry.source === 'server-template' ? 'Official server template' : (entry.category || 'Workflow folder')} | {TASK_LABELS[entry.task || 'generate']}
                                    </div>
                                </div>
                                {entry.registration ? (
                                    <button
