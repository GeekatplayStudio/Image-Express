...
import { bakeRelight, rebakeGlobalLightLayers, DEFAULT_AMBIENT, type LayerWithCache } from '@/lib/threeDLayer/bake';
...
export function ThreeDRelightControls({ canvas, layer, settings: settingsProp }: ThreeDRelightControlsProps) {
    ...
    // (relight AND object modes), not just the selected one.
    ...
    return (
        <div className="space-y-2.5">
            {settings.depthSource === 'fallback' && (
                <p className="text-[10px] text-amber-500/90">{t('layer3d.relight.fallbackDepth')}</p>
            )}
