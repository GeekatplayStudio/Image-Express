...
import { StarPolygon, ExtendedFabricObject, AdjustmentLayerType, AdjustmentLayerSettings, CurvesAdjustmentSettings, LevelsAdjustmentSettings, SaturationVibranceSettings, HueSaturationSettings, ExposureSettings, FabricBaseFilter, CurvesChannel } from '@/types';
...
class CurvesFilter extends fabric.filters.BaseFilter<'Curves', { points: CurvePoint[]; channel?: CurvesChannel; intensity?: number }, { points: CurvePoint[]; channel?: CurvesChannel; intensity?: number }> {
    static type = 'Curves';
    ...
    applyTo2d(options: { imageData: ImageData }) {
        ...
        for (let i = 0; i < data.length; i += 4) {
            ...
            if (channel === 'r') {
                ...
                // Luminosity preserving color shift is complex. 
                // A simpler "Luminosity" blending mode style:
                // Calculate lum, map it, apply diff to RGB?
                // Standard Curves "Luminosity" channel usually maps RGB by same curve but preserves saturation?
                // Or does it mean apply curve to Luminosity component?
                // Let's assume standard RGB mapping but weighting?
                // A common "Simple" approach for curves luminosity is:
                // data[i] = lut[r]; data[i+1] = lut[g]; data[i+2] = lut[b]; 
                // but that's RGB master.
                // If channel is 'RGB' (Master), we do:
...
export default function PropertiesPanel({ canvas, activeTool, onMake3D, onLayerDblClick, onPreviewMedia }: PropertiesPanelProps) {
    ...
    const getAdjustmentLabel = useCallback((type?: AdjustmentLayerType) => {
    ...
    const getDefaultAdjustmentSettings = useCallback((type: AdjustmentLayerType): AdjustmentLayerSettings => {
    ...
    const buildAdjustmentFilters = useCallback((type: AdjustmentLayerType, settings: AdjustmentLayerSettings, intensity: number) => {
        const filters: FabricBaseFilter[] = [];
        const clamp = (value: number, min: number, max: number) => Math.min(max, Math.max(min, value));
        const scaled = (value: number) => clamp(value * intensity, -1, 1);

        if (type === 'curves') {
            const curves = settings as CurvesAdjustmentSettings;
            const channel = curves.channel ?? 'rgb';
            const channelPoints = curves.pointsByChannel?.[channel];
            const points = channelPoints && channelPoints.length > 0
                ? channelPoints
                : (curves.points && curves.points.length > 0 ? curves.points : [{ x: 0, y: 0 }, { x: 1, y: 1 }]);
            filters.push(new CurvesFilter({ points, channel, intensity }) as unknown as FabricBaseFilter);
        }

        if (type === 'levels') {
            const { black, mid, white } = settings as LevelsAdjustmentSettings;
            const brightness = scaled((white - 1) - black);
            const contrast = scaled((white - black - 1));
            const gamma = clamp(mid, 0.2, 2);
        ...
        if (type === 'saturation-vibrance') {
            const { saturation, vibrance } = settings as SaturationVibranceSettings;
            filters.push(new fabric.filters.Saturation({ saturation: scaled(saturation) }));
            filters.push(new fabric.filters.Vibrance({ vibrance: scaled(vibrance) }));
        }

        if (type === 'hue-saturation') {
            const { hue, saturation, lightness } = settings as HueSaturationSettings;
            if (hue !== 0) {
                filters.push(new fabric.filters.HueRotation({ rotation: scaled(hue) }));
            }
            if (saturation !== 0) {
                filters.push(new fabric.filters.Saturation({ saturation: scaled(saturation) }));
            }
            if (lightness !== 0) {
                filters.push(new fabric.filters.Brightness({ brightness: scaled(lightness) }));
            }
        }

        if (type === 'exposure') {
            const { exposure, contrast } = settings as ExposureSettings;
            if (exposure !== 0) {
                filters.push(new fabric.filters.Brightness({ brightness: scaled(exposure) }));
            }
            if (contrast !== 0) {
                filters.push(new fabric.filters.Contrast({ contrast: scaled(contrast) }));
            }
        }
    ...
    const applyAdjustmentLayers = useCallback(() => {
        if (!canvas) return;
        if (isApplyingAdjustmentsRef.current) return;
        isApplyingAdjustmentsRef.current = true;

        try {
            const order = flatLayers;
            if (order.length === 0) return;

            const adjustments = order
                .map((node, index) => ({ node, index }))
                .filter(({ node }) => (node.obj as ExtendedFabricObject).isAdjustmentLayer);

            const shouldApplyWithinGroup = (parentId: string | null, imageNodeId: string) => {
                if (!parentId) return true;
                return isDescendant(parentId, imageNodeId);
            };

            order.forEach((node, index) => {
                if (node.obj.type !== 'image') return;
                const img = node.obj as fabric.Image;
                const extendedObj = node.obj as ExtendedFabricObject;

                if (!extendedObj.baseFilters) {
                    extendedObj.baseFilters = (img.filters ?? []).slice();
                }

                const filters: FabricBaseFilter[] = [...(extendedObj.baseFilters ?? [])];
                adjustments.forEach(({ node: adjNode, index: adjIndex }) => {
                    if (adjIndex > index) return; // apply layers above in the stack
                    if (!adjNode.obj.visible) return;
                    const adjustment = adjNode.obj as ExtendedFabricObject;
                    if (!adjustment.adjustmentType || !adjustment.adjustmentSettings) return;
                    if (!shouldApplyWithinGroup(adjNode.parentId, node.id)) return;
                    const intensity = typeof adjNode.obj.opacity === 'number' ? adjNode.obj.opacity : 1;
                    filters.push(...buildAdjustmentFilters(adjustment.adjustmentType, adjustment.adjustmentSettings, intensity));
                });
            ...
            isApplyingAdjustmentsRef.current = false;
        }
    }, [buildAdjustmentFilters, canvas, flatLayers, isDescendant]);
    ...
    const createAdjustmentLayer = useCallback((type: AdjustmentLayerType) => {
        ...
        extLayer.name = getNextIndexedName(getAdjustmentLabel(type), getGroupNames());
    ...
    }, [applyAdjustmentLayers, canvas, getAdjustmentLabel, getDefaultAdjustmentSettings, getGroupNames, getNextIndexedName]);
    ...
    useEffect(() => {
        ...
        (canvas.on as unknown as (eventName: string, handler: (payload?: { type?: AdjustmentLayerType }) => void) => void)('adjustment:create', handleCreate);
        return () => {
            (canvas.off as unknown as (eventName: string, handler: (payload?: { type?: AdjustmentLayerType }) => void) => void)('adjustment:create', handleCreate);
        };
    }, [canvas, createAdjustmentLayer]);

    const updateAdjustmentLayer = useCallback((nextSettings: AdjustmentLayerSettings) => {
        if (!canvas || !selectedObject) return;
        const target = selectedObject as ExtendedFabricObject;
        if (!target.isAdjustmentLayer || !target.adjustmentType) return;
        target.set('adjustmentSettings', nextSettings);
        setAdjustmentSettings(nextSettings);
        canvas.requestRenderAll();
        applyAdjustmentLayers();
    }, [applyAdjustmentLayers, canvas, selectedObject]);
    ...
    const isUpdatingObjectsRef = useRef(false);
    ...
    const syncCanvasMetrics = useCallback(() => {
        ...
        if (artboardInfo) {
            const nextWidth = Math.round(artboardInfo.width);
            const nextHeight = Math.round(artboardInfo.height);
            setCanvasWidth((prev) => (prev === nextWidth ? prev : nextWidth));
            setCanvasHeight((prev) => (prev === nextHeight ? prev : nextHeight));
        } else {
            const zoom = canvas.getZoom() || 1;
            const nextWidth = Math.round((canvas.width || 1080) / zoom);
            const nextHeight = Math.round((canvas.height || 1080) / zoom);
            setCanvasWidth((prev) => (prev === nextWidth ? prev : nextWidth));
            setCanvasHeight((prev) => (prev === nextHeight ? prev : nextHeight));
        }
        ...
        const nextPreset = presetMatch?.name ?? 'Custom';
        setSelectedPreset((prev) => (prev === nextPreset ? prev : nextPreset));

        const workspaceColor = normalizeColorValue(extendedCanvas.getWorkspaceBackground?.() || extendedCanvas.workspaceBackground);
        if (workspaceColor) {
            setWorkspaceBgColor((prev) => (prev === workspaceColor ? prev : workspaceColor));
            ...
            if (computed) {
                setWorkspaceBgColor((prev) => (prev === computed ? prev : computed));
            }
        }
    }, [canvas]);
    ...
    useEffect(() => {
        ...
        const updateObjects = () => {
            if (isUpdatingObjectsRef.current) return;
            isUpdatingObjectsRef.current = true;
            ...
            const nextObjects = [...objs].reverse();
            setObjects((prev) => {
                if (prev.length !== nextObjects.length) return nextObjects;
                for (let i = 0; i < prev.length; i += 1) {
                    const prevId = ensureObjectId(prev[i]);
                    const nextId = ensureObjectId(nextObjects[i]);
                    if (prevId !== nextId) return nextObjects;
                }
                return prev;
            });
            applyAdjustmentLayers();
            requestAnimationFrame(() => {
                isUpdatingObjectsRef.current = false;
            });
        };
        ...
        const handleSelection = (e: { selected?: fabric.Object[] } = {}) => {
            ...
            if (availableTarget && availableTarget.type !== 'activeSelection' && availableTarget.type !== 'group') {
                if ((availableTarget as ExtendedFabricObject).isAdjustmentLayer) {
                    const adj = availableTarget as ExtendedFabricObject;
                    const nextSettings = adj.adjustmentSettings ?? (adj.adjustmentType ? getDefaultAdjustmentSettings(adj.adjustmentType) : null);
                    setAdjustmentSettings(nextSettings);
                } else {
                    setAdjustmentSettings(null);
                }
    ...
    }, [canvas, ensureObjectId, applyAdjustmentLayers]);
    ...
    const updateImageFilter = (type: string, value: number) => {
        ...
        const extendedObj = activeObject as ExtendedFabricObject;

        // Initialize baseFilters if not present. This captures the state BEFORE adjustments are re-applied.
        // If adjustments were already applied, baseFilters should already exist.
        // If they don't exist, we assume current img.filters are the base filters (minus any adjustments if we could detect them, but simpler to assume fresh start).
        if (!extendedObj.baseFilters) {
            extendedObj.baseFilters = (img.filters ?? []).slice();
        }
        
        // We operate on baseFilters now
        const filters = extendedObj.baseFilters!;
        ...
        // Apply adjustments (which merges baseFilters + adjustment layers)
        // rather than just applying local filters.
        applyAdjustmentLayers();
        
        // However, applyAdjustmentLayers only loops through ALL layers.
        // It might be expensive but it guarantees consistency.
        // If applyAdjustmentLayers is debounced or gated, we might need to force it.
        // It is gated by ref, but should be fine.
        // Note: applyAdjustmentLayers calls canvas.requestRenderAll()
    };
    ...
    const toggleVisibility = (obj: fabric.Object) => {
        ...
        setObjects([...canvas.getObjects().reverse()]); 
        if ((obj as ExtendedFabricObject).isAdjustmentLayer) {
            applyAdjustmentLayers();
        }
    };
        ...
        return (
            <div className="flex flex-col h-full bg-card">
                ...
                <div className="px-3 py-2 border-b border-border/50">
                    ...
                    {selectedObject && (selectedObject as ExtendedFabricObject).isAdjustmentLayer && (
                        <div className="mt-4 space-y-3">
                            <div className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Adjustment Controls</div>
                            {(() => {
                                const adjustment = selectedObject as ExtendedFabricObject;
                                const type = adjustment.adjustmentType;
                                const settings = adjustmentSettings ?? adjustment.adjustmentSettings ?? (type ? getDefaultAdjustmentSettings(type) : undefined);
                                ...
                                if (type === 'curves') {
                                    ...
                                    const handleAddPoint = (event: { currentTarget: SVGSVGElement; clientX: number; clientY: number; button?: number; target: EventTarget | null }) => {
                                        ...
                                        updateAdjustmentLayer({ ...curves, channel, pointsByChannel: nextByChannel });
                                    };
                                            ...
                                            <div className="flex items-center justify-between gap-2">
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        const existing = curves.pointsByChannel ?? { [channel]: sorted };
                                                        updateAdjustmentLayer({ ...curves, channel: nextChannel, pointsByChannel: existing });
                                                    }}
                                        ...
                                        <div className="space-y-3">
                                            <div className="space-y-2">
                                                ...
                                                <input
                                                    ...
                                                    step="0.01"
                                                    value={levels.black}
                                                    onChange={(e) => updateAdjustmentLayer({ ...levels, black: parseFloat(e.target.value) })}
                                                    onDoubleClick={() => updateAdjustmentLayer({ ...levels, black: 0 })}
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <input
                                                    ...
                                                    step="0.01"
                                                    value={levels.mid}
                                                    onChange={(e) => updateAdjustmentLayer({ ...levels, mid: parseFloat(e.target.value) })}
                                                    onDoubleClick={() => updateAdjustmentLayer({ ...levels, mid: 1 })}
                                                    className="w-full h-1 bg-secondary rounded-lg appearance-none cursor-pointer"
                                                />
                                            </div>
                                            <div className="space-y-2">
                                                ...
                                                <input
                                                    ...
                                                    step="0.01"
                                                    value={levels.white}
                                                    onChange={(e) => updateAdjustmentLayer({ ...levels, white: parseFloat(e.target.value) })}
                                                    onDoubleClick={() => updateAdjustmentLayer({ ...levels, white: 1 })}
                                                    className="w-full h-1 bg-secondary rounded-lg appearance-none cursor-pointer"
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-3">
                                            <div className="space-y-2">
                                                ...
                                                <input
                                                    ...
                                                    step="0.01"
                                                    value={hueSat.hue}
                                                    onChange={(e) => updateAdjustmentLayer({ ...hueSat, hue: parseFloat(e.target.value) })}
                                                    onDoubleClick={() => updateAdjustmentLayer({ ...hueSat, hue: 0 })}
                                                    className="w-full h-1 bg-secondary rounded-lg appearance-none cursor-pointer"
                                                />
                                            </div>
                                            <div className="space-y-2">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => updateAdjustmentLayer({ ...hueSat, saturation: parseFloat(e.target.value) })}
                                                    onDoubleClick={() => updateAdjustmentLayer({ ...hueSat, saturation: 0 })}
                                                    className="w-full h-1 bg-secondary rounded-lg appearance-none cursor-pointer"
                                                />
                                            </div>
                                            <div className="space-y-2">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => updateAdjustmentLayer({ ...hueSat, lightness: parseFloat(e.target.value) })}
                                                    onDoubleClick={() => updateAdjustmentLayer({ ...hueSat, lightness: 0 })}
                                                    className="w-full h-1 bg-secondary rounded-lg appearance-none cursor-pointer"
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-3">
                                            <div className="space-y-2">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => updateAdjustmentLayer({ ...exposure, exposure: parseFloat(e.target.value) })}
                                                    onDoubleClick={() => updateAdjustmentLayer({ ...exposure, exposure: 0 })}
                                                    className="w-full h-1 bg-secondary rounded-lg appearance-none cursor-pointer"
                                                />
                                            </div>
                                            <div className="space-y-2">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => updateAdjustmentLayer({ ...exposure, contrast: parseFloat(e.target.value) })}
                                                    onDoubleClick={() => updateAdjustmentLayer({ ...exposure, contrast: 0 })}
                                                    className="w-full h-1 bg-secondary rounded-lg appearance-none cursor-pointer"
                                                />
                                            </div>
                                        </div>
                                    ...
                                    <div className="space-y-3">
                                        <div className="space-y-2">
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => updateAdjustmentLayer({ ...sat, saturation: parseFloat(e.target.value) })}
                                                onDoubleClick={() => updateAdjustmentLayer({ ...sat, saturation: 0 })}
                                                className="w-full h-1 bg-secondary rounded-lg appearance-none cursor-pointer"
                                            />
                                        </div>
                                        <div className="space-y-2">
                                            ...
                                            <input
                                                ...
                                                step="0.01"
                                                value={sat.vibrance}
                                                onChange={(e) => updateAdjustmentLayer({ ...sat, vibrance: parseFloat(e.target.value) })}
                                                onDoubleClick={() => updateAdjustmentLayer({ ...sat, vibrance: 0 })}
                                                className="w-full h-1 bg-secondary rounded-lg appearance-none cursor-pointer"
                                            />
                                        </div>
                                    </div>
