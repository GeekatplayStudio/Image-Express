...
type ThreeDLayerEditorInitialState = {
...
const getInitialThreeDLayerEditorState = (existingObject?: fabric.Object): ThreeDLayerEditorInitialState => {
    const defaults: ThreeDLayerEditorInitialState = {
    ...
    if (settings) {
        return {
            resolution: settings.resolution ? { ...settings.resolution } : { ...defaults.resolution },
            lightPosition: settings.lightPosition ? { ...settings.lightPosition } : { ...defaults.lightPosition },
            lightIntensity: settings.lightIntensity ?? defaults.lightIntensity,
            lightColor: settings.lightColor ?? defaults.lightColor,
            castShadowEnabled: settings.castShadowEnabled ?? defaults.castShadowEnabled,
            castShadowBlur: settings.castShadowBlur ?? defaults.castShadowBlur,
            castShadowIntensity: settings.castShadowIntensity ?? defaults.castShadowIntensity,
            contactShadowEnabled: settings.contactShadowEnabled ?? defaults.contactShadowEnabled,
            contactShadowBlur: settings.contactShadowBlur ?? defaults.contactShadowBlur,
            contactShadowIntensity: settings.contactShadowIntensity ?? defaults.contactShadowIntensity,
        };
    }
...
const ThreeDLayerEditor = ({ modelUrl, existingObject, onSave, onClose }: ThreeDLayerEditorProps) => {
    const [initialSettings] = useState(() => getInitialThreeDLayerEditorState(existingObject));
    const captureGlRef = useRef<CaptureGL | null>(null);
    const [captureGlVersion, setCaptureGlVersion] = useState(0);
    const [resolution, setResolution] = useState<{ width: number, height: number }>(() => initialSettings.resolution);
    const [showResSettings, setShowResSettings] = useState(false);
    const [showLightSettings, setShowLightSettings] = useState(false);
    const [lightPosition, setLightPosition] = useState<{ x: number; y: number; z: number }>(() => initialSettings.lightPosition);
    const [lightIntensity, setLightIntensity] = useState(() => initialSettings.lightIntensity);
    const [lightColor, setLightColor] = useState(() => initialSettings.lightColor);
    const [castShadowEnabled, setCastShadowEnabled] = useState(() => initialSettings.castShadowEnabled);
    const [castShadowBlur, setCastShadowBlur] = useState(() => initialSettings.castShadowBlur);
    const [castShadowIntensity, setCastShadowIntensity] = useState(() => initialSettings.castShadowIntensity);
    const [contactShadowEnabled, setContactShadowEnabled] = useState(() => initialSettings.contactShadowEnabled);
    const [contactShadowBlur, setContactShadowBlur] = useState(() => initialSettings.contactShadowBlur);
    const [contactShadowIntensity, setContactShadowIntensity] = useState(() => initialSettings.contactShadowIntensity);
    ...
    useEffect(() => {
        ...
        const extended = existingObject as ExtendedFabricObject;
        const settings = extended.threeDSettings;
        const captureGl = captureGlRef.current;
        if (!settings || !captureGl || !controlsRef.current) return;

        const { cameraPosition, cameraTarget } = settings;
        if (cameraPosition) {
            captureGl.camera.position.set(cameraPosition.x, cameraPosition.y, cameraPosition.z);
        }
        if (cameraTarget) {
    ...
    }, [existingObject, captureGlVersion]);
    ...
    const handleCapture = () => {
        const captureGl = captureGlRef.current;
        if (captureGl && captureGl.glInstance) {
            const { glInstance: renderer, scene } = captureGl;
            const camera = captureGl.camera as THREE.PerspectiveCamera;
            try {
                ...
                captureGl.render();
                const dataUrl = captureGl.domElement.toDataURL('image/png', 1.0);
                const controls = controlsRef.current;
                const settings: ThreeDSettings = {
                    lightPosition,
                    lightIntensity,
                ...
                >
                    ...
                    <Canvas
                        ...
                        onCreated={({ gl, scene, camera }) => {
                            ...
                            setCaptureGlVersion((current) => current + 1);
                        }}
                    >
