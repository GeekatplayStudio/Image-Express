...
interface ImageGeneratorModalProps {
  ...
  onClose: () => void;
    /** Opens the full settings modal to configure provider credentials/endpoints */
    onOpenSettings?: () => void;
...
export default function ImageGeneratorModal({
  ...
  const [isCheckingComfyConnection, setIsCheckingComfyConnection] = useState(false);
    ...
    const [isLoadingComfyInstallerStatus, setIsLoadingComfyInstallerStatus] = useState(false);
  const [comfyConnectionStatusMessage, setComfyConnectionStatusMessage] = useState('');
    ...
    const [comfyInstallerStatus, setComfyInstallerStatus] = useState<InstallerRuntimeStatus | null>(null);
  ...
  const syncComfyInstallerStatus = useCallback(async (): Promise<InstallerRuntimeStatus | null> => {
      setIsLoadingComfyInstallerStatus(true);
      try {
          const status = await fetchInstallerRuntimeStatus(comfyInstallPath.trim());
          setComfyInstallerStatus(status);
          return status;
      } catch {
          setComfyInstallerStatus(null);
          return null;
      } finally {
          setIsLoadingComfyInstallerStatus(false);
      }
  }, [comfyInstallPath]);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        ...
        const providers = GENERATIVE_PROVIDER_OPTIONS.map((provider) => provider.id);
        ...
        setComfyTunnelUrl(preferences.comfyTunnelUrl);
  ...
  const ensureComfyConnectionReady = useCallback(async (): Promise<boolean> => {
      const verification = await verifyAvailableComfyConnection({
          mode: comfyConnectionMode,
          localUrl: comfyServerUrl,
          tunnelUrl: comfyTunnelUrl,
      ...
      if (!verification.ok) {
          setComfyCatalogStatusMessage('');
          setComfyMissingRequirements(null);
          setStatusMessage(`Error: ${verification.message}`);
          return false;
      }
  ...
  const hasComfyConfigForCatalogSync = isComfyConnectionConfigured(comfyConnectionMode, {
      localUrl: comfyServerUrl,
      tunnelUrl: comfyTunnelUrl,
      cloudUrl: comfyCloudUrl,
      cloudApiKey: comfyCloudApiKey,
  ...
  useEffect(() => {
      if (!isOpen || selectedProvider !== 'comfy') {
          return;
      }

      void syncComfyInstallerStatus();
  }, [isOpen, selectedProvider, syncComfyInstallerStatus]);
  ...
  const isProviderConfigured = (provider: GenerativeProviderId): boolean => {
      if (provider === 'comfy') {
      ...
      if (provider === 'stability') {
          return Boolean(apiKey || getProviderKey('stability'));
      }
    ...
    const recoveryToken = ++comfyRunTokenRef.current;
      ...
      void recoverComfyTaskByPromptId({
          ...
          connection: {
              ...
              tunnelUrl: pending.connection.tunnelUrl,
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (selectedProvider === 'comfy') {
          ...
          const comfyConnectionReady = await ensureComfyConnectionReady();
          if (!comfyConnectionReady) {
              setIsGenerating(false);
              return;
          }
      ...
      setIsGenerating(false);
            ...
            const isHandledComfyConnectionFailure = selectedProvider === 'comfy' && isComfyConnectionFailureMessage(message);
            if (!isAbortError && !isHandledComfyConnectionFailure) {
                console.error(error);
            }
                        if (isAbortError) {
                                ...
                                if (missingModel) {
                                    ...
                                    if (isHandledComfyConnectionFailure) {
                                        setComfyConnectionStatusMessage(message);
                                    }
                                    if (selectedProvider === 'comfy' && isComfyModelAvailabilityError(message)) {
                                        const installerStatus = comfyInstallerStatus || await syncComfyInstallerStatus();
                                        const guidance = installerStatus
                                            ? buildComfyModelAvailabilityGuidance(message, installerStatus, comfyEndpointLabel)
                                            : null;

                                        setStatusMessage(`Error: ${guidance ? `${message} ${guidance}` : message}`);
                                    } else {
                                        setStatusMessage(`Error: ${message}`);
                                    }
                                }
    ...
    const hasStabilityAccess = Boolean(apiKey || getProviderKey('stability'));
    ...
    <div className="flex-1 min-h-0 overflow-y-auto bg-background p-3 space-y-3 relative">
        ...
        >
            ...
            {!isSelectedProviderConfigured && isSelectedProviderReady && (
                <div className="space-y-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-[10px] text-amber-700 dark:text-amber-300">
                    <div>{selectedProviderSetupMessage}</div>
                    {onOpenSettings && (
                        <button
                            type="button"
              ...
              {selectedProvider === 'comfy' && (
                   ...
                   >
                       <div className="space-y-1">
                           ...
                           >
                               {COMFY_CONNECTION_MODE_OPTIONS.map((option) => (
                           ...
                           <div className="text-[10px] text-muted-foreground rounded border border-border/60 bg-background/40 px-2 py-1.5">
                               {getComfyConnectionModeDescription(comfyConnectionMode)}
                           </div>
                       </div>
                       ...
                       <div className="space-y-1">
                           ...
                           <input
                               ...
                               onChange={(event) => {
                                   const nextUrl = event.target.value;
                                   setComfyTunnelUrl(nextUrl);
                                   saveGenerativePreferences({ comfyTunnelUrl: nextUrl });
                               }}
                               ...
                               <>
                                   ...
                                   {comfyInstallPath.trim() !== comfyInstallerStatus.comfyDirectory.path && (
                                       <button
                                           type="button"
                                           onClick={() => {
                                               const nextPath = comfyInstallerStatus.comfyDirectory.path;
                                               setComfyInstallPath(nextPath);
                                               saveGenerativePreferences({ comfyInstallPath: nextPath });
                                           }}
