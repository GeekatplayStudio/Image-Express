            ...
            <div className="relative z-[130]" ref={exportRef}>
                ...
                {showExportMenu && (
                    <div className="absolute right-0 top-full mt-2 w-[320px] bg-card border border-border/50 rounded-xl shadow-xl overflow-hidden py-1 animate-in fade-in slide-in-from-top-2 z-[170]">
                        ...
                        <button onClick={() => handleExport('cricut')} className="w-full text-left px-4 py-2.5 text-sm hover:bg-secondary/50 flex items-center gap-3"><ScanLine size={16} className="text-emerald-500" /> <span className="font-medium">{t('hdr.cricut')}</span></button>
                    </div>
                )}
            </div>
