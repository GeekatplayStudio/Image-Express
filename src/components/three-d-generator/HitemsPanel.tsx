            ...
            <div className="space-y-2 rounded-md border border-border/60 bg-secondary/30 p-3">
                ...
                <div className="space-y-1">
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-1">
                        ...
                        <div className="space-y-1"><div className="flex items-center justify-between gap-2"><p className="text-[10px] text-muted-foreground">Back Preview</p>{hitemsImageViewMode !== 'multi' && <span className="text-[9px] uppercase tracking-wide text-muted-foreground">Saved for multi-view</span>}</div><div className="relative h-16 rounded border border-border/50 bg-black/10 overflow-hidden">{resolveLayerImageUrl(hitemsBackLayerId) ? <Image src={resolveLayerImageUrl(hitemsBackLayerId)} alt="Back layer preview" fill sizes="128px" className="object-contain" unoptimized /> : <div className="h-full w-full flex items-center justify-center text-[10px] text-muted-foreground">Not set</div>}</div></div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-1 pt-1">
                        ...
                        <select value={hitemsBackLayerId} onChange={(event) => onBackLayerChange(event.target.value)} className="w-full text-xs p-2 rounded bg-secondary/50 border border-border"><option value="">Back: Not set</option>{normalizedLayerImageOptions.map((option) => <option key={`always-back-${option.id}`} value={option.id}>{option.label}</option>)}</select>
                        {hitemsImageViewMode !== 'multi' && <p className="text-[10px] text-muted-foreground">Back artwork stays visible here and will be included automatically when you switch to Multi-view.</p>}
                    </div>
