...
import type { ComfyDiagnosticsSnapshot, ComfyLibraryRepoKind, ComfyLibrarySnapshot } from '@/lib/comfyui/libraryTypes';
...
import {
    runDependencyRuntime,
    type DependencyRunResult,
...
export default function SettingsModal({ isOpen, onClose, userId, userRoles }: SettingsModalProps) {
    ...
    const [comfySetupCheck, setComfySetupCheck] = useState<{
        state: 'idle' | 'checking' | 'success' | 'error';
        message: string;
    }>({
        state: 'idle',
    ...
    const autoDetectedComfyPathsRef = useRef({
        installPath: '',
        customNodesPath: '',
        workflowLibraryPath: '',
    });
    ...
    const handleVerifyLocalComfySetup = useCallback(async () => {
        setComfySetupCheck({
            state: 'checking',
            message: 'Checking local ComfyUI runtime and configured folders...',
        });
        setComfyDiagnostics(null);
        setComfyMissingRequirements(null);

        try {
            const localVerification = await verifyAvailableComfyConnection({
                mode: 'local',
                localUrl: comfyServerUrl,
                tunnelUrl: comfyTunnelUrl,
            });
            ...
            setComfyConnectionCheck({
                state: localVerification.ok ? 'success' : 'error',
            ...
            diagnostics.paths.statuses.forEach((status) => {
                if (!status.exists || !status.readable) {
                    issueKeys.add(`${status.label}:${status.path}`);
                }
            });
            nextInstallerStatus.paths.statuses.forEach((status) => {
                if (!status.exists) {
                    issueKeys.add(`${status.label}:${status.path}`);
                }
            });
            ...
            setComfySetupCheck({
                state: 'error',
                message: error instanceof Error ? error.message : 'Failed to verify the local ComfyUI setup.',
            });
        }
    }, [
    ...
    useEffect(() => {
        ...
        const previousAutoDetected = autoDetectedComfyPathsRef.current;
        ...
        if (detectedCustomNodesPath) {
            setComfyCustomNodesPath((current) => {
                const trimmed = current.trim();
    ...
    const loadDependencyStatus = useCallback(async () => {
        ...
        try {
            ...
            setDependencyStatusMessage(
                error instanceof Error ? error.message : 'Failed to load dependency maintenance status.',
            );
        }
    }, []);
    ...
    const handleRunDependencyMaintenance = useCallback(async () => {
        ...
        try {
            const result = await runDependencyRuntime({
                strategy: 'latest',
                runBuild: true,
            });
    ...
    useEffect(() => {
        if (!isOpen || activeSettingsTab !== 'workspace') {
            return;
        }
                    ...
                    <section className={`${accentSectionClass} xl:col-span-12`}>
                        ...
                        <div className="space-y-3 rounded-xl border border-border/60 bg-background/60 p-3">
                            <div className="flex flex-wrap gap-2">
                                <button
                                    onClick={() => void handleVerifyLocalComfySetup()}
                                    disabled={comfySetupCheck.state === 'checking'}
                                ...
                                >
                                    {comfySetupCheck.state === 'checking' ? (
                                        <Loader2 size={13} className="animate-spin" />
                                    ) : (
                                        <ShieldCheck size={13} />
                                    )}
                            ...
                            {comfySetupCheck.message && (
                                <div
                                    className={`text-[11px] rounded-md border px-2.5 py-2 ${
                                        comfySetupCheck.state === 'success'
                                            ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-700'
                                            : comfySetupCheck.state === 'error'
                            ...
                            {comfyDiagnostics?.paths?.statuses?.length ? (
                                <div className="rounded-md border border-border/60 bg-background/70 px-3 py-2 space-y-2 text-[11px]">
                                    ...
                                    <div className="space-y-2">
                                        {comfyDiagnostics.paths.statuses.map((status) => {
