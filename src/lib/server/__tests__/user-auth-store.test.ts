...
describe('user-auth-store', () => {
    ...
    it('seeds default admin user when store is missing', async () => {
        const store = await loadUsers();

        expect(store.users).toHaveLength(1);
        const admin = store.users.find((user) => user.email === 'admin@local');
    ...
    it('supports helper utilities for public user shaping and lookup', async () => {
        ...
        const admin = seeded.users.find((user) => user.email === 'admin@local');
        expect(admin).toBeDefined();

        const byEmail = findUserByIdentifier(seeded.users, 'ADMIN@LOCAL');
        expect(byEmail?.email).toBe('admin@local');

        const byUsername = findUserByIdentifier(seeded.users, 'admin');
        expect(byUsername?.email).toBe('admin@local');
        ...
        expect(publicUser.email).toBe('admin@local');
