...
| 29 | Reference design upgrade program (menus/submenus/properties/tools) | In Progress (Phase 1 MVP + C2/C3 + C4.1/C4.2/C4.3/C4.4 + C5.1/C5.2/C5.3/C5.4/C5.5/C5.6 complete) | src/components/Editor/TopToolOptionsBar.tsx, src/components/Editor/EditorView.tsx, src/components/Toolbar.tsx, src/components/Editor/__tests__/TopToolOptionsBar.test.tsx, src/components/Editor/__tests__/EditorView.test.tsx, docs/imageprocessingui_upgrade_execution_checklist.md | Pass | Pass | Added TopToolOptionsBar MVP command layer, implemented C2 select controls (including C2.4 Feather and Anti-alias with active-object feather shadow application + canvas smoothing toggle), implemented C3 brush/paint quick controls (preset, size, hardness, opacity, flow, smoothing, blend mode) with Fabric brush wiring, implemented C4.1 pen Path/Shape mode toggles, implemented C4.2 Add/Subtract/Intersect operations mapped through existing pen event/config flow (`pen:config:set`/`pen:draft:update`) and applied on created pen paths via `globalCompositeOperation`, implemented C4.3 Auto add/delete toggle (top bar + pen config flow + anchor click delete behavior during pen drafting), implemented C4.4 Rubber band toggle with top-bar wiring through pen config events and live draft line preview-to-cursor behavior in pen mode, implemented C5.1 Text Font Family selector in top bar with active text sync/apply behavior, implemented C5.2 Text Font Style selector (mapped to text `fontWeight`) with active text sync/apply behavior, implemented C5.3 Text Size control with active text sync/apply behavior, implemented C5.4 Bold/Italic/Underline toggles with active text sync/apply behavior, implemented C5.5 Text Alignment controls (left/center/right/justify) with active text sync/apply behavior, and implemented C5.6 Text color shortcut with active text fill sync/apply behavior. Also scoped lint/build to ignore `Imageprocessingui/**` reference repo via eslint/tsconfig. |
